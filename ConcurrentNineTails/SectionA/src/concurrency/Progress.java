...
import concurrency.expressions.IdentifierExpr;
import concurrency.expressions.LiteralExpr;
import concurrency.schedulers.FewestWaitsScheduler;
import concurrency.schedulers.RoundRobinScheduler;
import concurrency.schedulers.Scheduler;
import concurrency.statements.AssignStmt;
...
public class Progress {
   ...
   * <p>
   * Store: { x == 0, y == 0 } Thread 0: x = 1; y = 2; Thread 1: x = 2; y = 3; Thread 2: wait(x, 2);
   * y = 4;
   * <p>
   ...
   * <p>
   * Store: { x == 0, y == 0, z == 0 } Thread 0: wait(z, 1); Thread 1: wait(y, 1); z = 1; Thread 2:
   * wait(x, 1); y = 1; Thread 3: x = 1;
   * <p>
   * Uncomment this method as needed when you uncomment the test methods
   *
   ...
   * <strong>Creates the following concurrent program:</strong>
   * <p>
   * Store: { x == 0, y == 0 } Thread 0: wait(x, 0); wait(x, 0); y = 1; Thread 1: wait(x, 0);
   * wait(x, 0); wait(x, 0); y = 2; Thread 2: wait(x, 0); wait(x, 0); wait(x, 0); y = 3; Thread 3:
   * wait(x, 0); wait(x, 0); y = 4;
   * <p>
   * Uncomment this method as needed when you uncomment the test methods
   *
   * @return an example concurrent program
